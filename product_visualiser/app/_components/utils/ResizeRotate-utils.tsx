...
export const getAngle = (point1: {x: number, y:number}, point2: {x: number, y:number}) => {
...
export const getNewStyle = (
  type: string,
  rect: {width: number, height: number, centerX: number, centerY: number, rotateAngle: number},
  deltaW: number,
  deltaH: number,
  ratio: number,
  minWidth: number,
...
export const getCursor = (rotateAngle: number, d: string) => {
  const mapValue = Math.floor(rotateAngle / 30);
  const increment = cursorMap[mapValue as keyof typeof cursorMap];
  const index = cursorStartMap[d as keyof typeof cursorStartMap];
...
export const tLToCenter = (
  ...
  :
  {top: number, left: number, width: number, height: number, rotateAngle: number}) => ({
  position: {
